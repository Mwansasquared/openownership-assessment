...
export function SubmissionDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([submissionsApi.get(id), submissionsApi.listEvents(id)])
      .then(([s, evts]) => {
        setSub(s); setTitle(s.title); setContent(s.content); setEvents(evts)
      })
  ...
  const saveEdit = async () => {
    if (!id) return
    const updated = await submissionsApi.update(id, title, content)
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <div className="fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
        ...
        {editing && (
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            ...
            <button onClick={() => setEditing(false)} style={btnStyle('default')}>Cancel</button>
          </div>
        )}
      </div>
      ...
      <div className="fade-up-1" style={{
        ...
        {sub.created_at !== sub.updated_at && (
          <>
            <span style={{ color: 'var(--border)', fontSize: 16 }}>·</span>
            <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>Updated {new Date(sub.updated_at).toLocaleString()}</span>
          </>
        )}
      </div>
      ...
      <div className="fade-up-1" style={{
        ...
        {editing ? (
          <textarea value={content} onChange={e => setContent(e.target.value)}
            rows={8} style={{ resize: 'vertical', minHeight: 160 }} />
        ) : (
          <p style={{
